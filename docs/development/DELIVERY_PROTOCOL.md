# OFARM2 Delivery Protocol

This procedure is incorporated by the root `AGENTS.md` for the activities it
specifies. It is not a second source of authority, a new OFARM law, or an
independent grant of permission. Root-file summaries do not waive these rules.
Read the complete applicable sections, including their prerequisites, before
performing or evaluating the corresponding action. Reviewing a mechanism
requires its applicable procedure even when no action will be executed. If
required text or approval evidence is unavailable, do not perform the affected
governed action or claim to have verified its missing requirements.

Paths mentioned below are repository-root-relative. Code-excellence
invariants and review classifications remain in [root AGENTS.md](../../AGENTS.md).
...
extracted text are limited to redirecting “review classifications below” to
root `AGENTS.md` and inserting the `Content review and re-review` and
`Baseline admission and evidence publication` subheadings. The new headings
make existing sections individually addressable; no operative clauses are
removed, reordered, or relaxed. Approval forms, closed precedence, trust
...
### Baseline admission and evidence publication
...
for any reason. The review classifications in root `AGENTS.md` constrain
reviewers, not the user's final decision.
