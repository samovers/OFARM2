...
| SI-VIEWS-001 | `views/OFARM_QuerySpecification_si_ffs_*` and related view artifacts | SI profile query specifications and query plans are stored under top-level `views/` | Authored SI views are profile material, but consumers and docs may currently expect the top-level path. | `MOVE_TO_SI_PROFILE` |
