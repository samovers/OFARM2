...
def test_p4_resolved_binding_attaches_to_committed_appliedresource_identity(store):
    ...
    with selected_runtime(store) as runtime:
        ...
        with runtime.serialized_tx() as cur:
            r = sib.resolve_product_authorisation(
                runtime, cur, pr, decision, ar_ref, created_by=demo.FARMER,
        ...
        result = GatePipeline(runtime).commit(
            demo.structure_submission(payload, idem_key=f"m2p4-bind:{s}"))
    assert result["decisionOutcome"] == "PROMOTE_ACCEPTED"


def test_p4_binding_for_a_different_subject_does_not_promote(store):
    ...
    with selected_runtime(store) as runtime:
        ...
        with runtime.serialized_tx() as cur:
            r = sib.resolve_product_authorisation(
                runtime, cur, pr, decision, f"resource:OTHER.{s}",
        ...
        result = GatePipeline(runtime).commit(
            demo.structure_submission(payload, idem_key=f"m2p4-mm:{s}"))
    assert result["decisionOutcome"] != "PROMOTE_ACCEPTED", \
...
def test_p4_fake_evidence_ref_produces_no_binding(store):
    ...
    with selected_runtime(store) as runtime:
        ...
        with runtime.serialized_tx() as cur:
            r = sib.resolve_product_authorisation(
                runtime, cur, pr, decision, "resource:f",
