...
| `profiles/si_ffs/runtime_provider.py` / `profiles/si_ffs/outputs.py` / `profiles/si_ffs/manifest_inputs.py` | SI-owned service graph, materialization/output identities, capability-manifest inputs, empty profile-evidence declaration, PassportView, and DocumentAssembly assembly; SI is the only production registration |
...
| `security_audit_gap.py` | bounded live-process observation and audit-control reconciliation of recovered pre-tenant evidence gaps; process-crash and store-loss operations remain separate deployment primitives |
...
| `manifest.py` | Profile-neutral Capability Manifest + ActiveArtifactSet assembly from the verified provider and actual runtime surfaces, plus grounding verification |
...
Automatic crash detection, crash/store-loss orchestration, retention
orchestration, and readiness orchestration · mobile app (M3) · registry adapter
scheduling · dynamic packs · public query compiler · AI/agent runtime · everything in
