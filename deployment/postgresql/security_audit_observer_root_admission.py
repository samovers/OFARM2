...
_JSON_ENUMS = {
    "denyAccessState": {"DENY_ACCESS_STATE_DENIED", "DENY_ACCESS_STATE_NOT_DENIED"},
    "membership": {"MEMBERSHIP_MATCHED", "MEMBERSHIP_NOT_MATCHED"},
...
_DENY_POLICY_NAME = (
    r"policies/cloudresourcemanager\.googleapis\.com%2F"
    r"(?:projects|folders|organizations)%2F[1-9][0-9]*/"
    r"denypolicies/[a-z](?:[a-z0-9.-]{1,61}[a-z0-9])"
)
_DENY_PERMISSION = (
    r"[a-z][a-z0-9-]*(?:\.[a-z0-9-]+)*\.googleapis\.com/"
    r"(?:[A-Za-z][A-Za-z0-9]*|\*)\.(?:[A-Za-z][A-Za-z0-9]*|\*)"
)
_DENY_PRINCIPAL = (
    r"(?:principal://goog/subject/[^/?#\s]+"
    r"|deleted:principal://goog/subject/[^/?#\s]+\?uid=[1-9][0-9]*"
    r"|principal://iam\.googleapis\.com/projects/-/serviceAccounts/[^/?#\s]+"
    r"|deleted:principal://iam\.googleapis\.com/projects/-/serviceAccounts/"
    r"[^/?#\s]+\?uid=[1-9][0-9]*"
    r"|principalSet://goog/group/[^/?#\s]+"
    r"|deleted:principalSet://goog/group/[^/?#\s]+\?uid=[1-9][0-9]*"
    r"|principalSet://goog/(?:public:all|cloudIdentityCustomerId/[A-Za-z0-9]+)"
    r"|(?:deleted:)?principal://iam\.googleapis\.com/locations/global/"
    r"workforcePools/[^/?#\s]+/subject/[^/?#\s]+"
    r"|principalSet://iam\.googleapis\.com/"
    r"(?:locations/global/workforcePools/[^/?#\s]+|"
    r"projects/[1-9][0-9]*/locations/global/workloadIdentityPools/[^/?#\s]+)/"
    r"(?:group/[^/?#\s]+|attribute\.[A-Za-z0-9_]+/[^/?#\s]+|\*)"
    r"|principal://iam\.googleapis\.com/projects/[1-9][0-9]*/locations/global/"
    r"workloadIdentityPools/[^/?#\s]+/subject/[^/?#\s]+"
    r"|principalSet://cloudresourcemanager\.googleapis\.com/"
    r"(?:projects|folders|organizations)/[1-9][0-9]*/"
    r"type/(?:ServiceAccount|ServiceAgent)"
    r"|principal://[a-z0-9.-]+\.system\.id\.goog/resources/"
    r"[A-Za-z0-9._~!$&'()+,;=:@%/-]+"
    r"|principalSet://[a-z0-9.-]+\.system\.id\.goog/"
    r"(?:\*|attribute\.platformContainer/[A-Za-z0-9._~!$&'()+,;=:@%/-]+))"
)


class _KmsObserverClient(Protocol):
    def get_crypto_key(
        self, *, request: kms_v1.GetCryptoKeyRequest, retry: None, timeout: float
    ) -> kms_v1.CryptoKey:
        # Protocol declaration.
        ...

    def get_crypto_key_version(
        self, *, request: kms_v1.GetCryptoKeyVersionRequest, retry: None, timeout: float
    ) -> kms_v1.CryptoKeyVersion:
        # Protocol declaration.
        ...

    def get_public_key(
        self, *, request: kms_v1.GetPublicKeyRequest, retry: None, timeout: float
    ) -> kms_v1.PublicKey:
        # Protocol declaration.
        ...
...
class _KmsSignerClient(Protocol):
    def asymmetric_sign(
        self, *, request: kms_v1.AsymmetricSignRequest, retry: None, timeout: float
...
def _members(
    value: object, required: tuple[str, ...], optional: tuple[str, ...] = ()
) -> dict[str, object]:
...
def _manifest(value: object) -> _Manifest:
    ...
    if (
        document["schemaVersion"] != _MANIFEST_SCHEMA
        or type(resource) is not str
        or fullmatch(_RESOURCE_PATTERN, resource) is None
        or type(signer) is not str
        or fullmatch(_PRINCIPAL_PATTERN, signer) is None
        or type(observer) is not str
        or fullmatch(_PRINCIPAL_PATTERN, observer) is None
        or signer == observer
        or type(digest_text) is not str
        or fullmatch(r"[0-9a-f]{64}", digest_text) is None
        or attestation_format not in {"CAVIUM_V1_COMPRESSED", "CAVIUM_V2_COMPRESSED"}
    ):
        raise ValueError
    ...
    etags = tuple(
        document[name]
        for name in ("signerRoleEtag", "observerVersionRoleEtag", "observerKeyRoleEtag")
    )
    for etag in etags:
        _b64_standard(etag)
...
def _binding_specs(manifest: _Manifest) -> tuple[_BindingSpec, ...]:
    ...
    return (
        _BindingSpec(
            prefix + "SignerV1",
            "serviceAccount:" + manifest.signer,
            ("cloudkms.cryptoKeyVersions.useToSign",),
            "ofarm-security-audit-observer-root-signer-v1",
            version_expression,
        ),
        _BindingSpec(
            prefix + "VersionReaderV1",
            "serviceAccount:" + manifest.observer,
            (
...
def _access_specs(
    manifest: _Manifest, bindings: tuple[_BindingSpec, ...]
) -> tuple[_AccessSpec, ...]:
...
def _crypto_key(client: _KmsObserverClient, manifest: _Manifest) -> dict[str, object]:
    ...
    if (
        value.name != manifest.key
        or value.purpose != kms_v1.CryptoKey.CryptoKeyPurpose.ASYMMETRIC_SIGN
        or value.import_only is not False
        or "primary" in value
        or "rotation_period" in value
        or "next_rotation_time" in value
        or "key_access_justifications_policy" in value
    ...
    return {
        "importOnly": False,
        "keyAccessJustificationsPolicyPresent": False,
        "name": manifest.key,
...
def _crypto_key_version(
    client: _KmsObserverClient, manifest: _Manifest
) -> dict[str, object]:
    request = kms_v1.GetCryptoKeyVersionRequest(name=manifest.resource)
    value = client.get_crypto_key_version(
        request=request, retry=None, timeout=_KMS_TIMEOUT
    )
    ...
    if (
        value.name != manifest.resource
        or value.state != kms_v1.CryptoKeyVersion.CryptoKeyVersionState.ENABLED
        or value.algorithm
        != kms_v1.CryptoKeyVersion.CryptoKeyVersionAlgorithm.EC_SIGN_ED25519
        or value.protection_level != kms_v1.ProtectionLevel.HSM
        or value.reimport_eligible is not False
        or "import_job" in value
        or "import_time" in value
        or attestation.format not in formats
        or formats[attestation.format] != manifest.attestation_format
        or type(content) is not bytes
        or not 1 <= len(content) <= _MAX_ATTESTATION
    ):
        raise ValueError
    bundle = {
        "caviumCerts": _certificate_chain(chains.cavium_certs),
        "content": b64encode(content).decode("ascii"),
        "format": manifest.attestation_format,
        "googleCardCerts": _certificate_chain(chains.google_card_certs),
        "googlePartitionCerts": _certificate_chain(chains.google_partition_certs),
    }
    if (
        sha256(_ATTESTATION_DOMAIN + _canonical(bundle)).digest()
...
def _public_key(client: _KmsObserverClient, manifest: _Manifest) -> dict[str, object]:
    request = kms_v1.GetPublicKeyRequest(
        name=manifest.resource, public_key_format=kms_v1.PublicKey.PublicKeyFormat.DER
    )
    ...
    if (
        ...
        != kms_v1.CryptoKeyVersion.CryptoKeyVersionAlgorithm.EC_SIGN_ED25519
        or value.protection_level != kms_v1.ProtectionLevel.HSM
        or value.public_key_format != kms_v1.PublicKey.PublicKeyFormat.DER
        or "pem" in value
        or "pem_crc32c" in value
        or type(der) is not bytes
        or der != bytes.fromhex("302a300506032b6570032100") + manifest.public_key
        or "crc32c_checksum" not in checked
        or type(checksum) is not int
        or isinstance(checksum, bool)
        or not 0 <= checksum <= 0xFFFFFFFF
        or checksum != _crc32c(der)
    ):
        raise ValueError
    return {
        "algorithm": "EC_SIGN_ED25519",
        ...
        "publicKey": {
            ...
            "data": b64encode(der).decode("ascii"),
        },
        "publicKeyFormat": "DER",
    }
...
def _role(
    session: _EvidenceSession, name: str, permissions: tuple[str, ...], etag: str
) -> dict[str, object]:
...
def _policy_binding(value: object) -> dict[str, object]:
    ...
    if (
        fullmatch(
            r"(?:projects|organizations)/[^/]+/roles/[A-Za-z0-9_.-]+|roles/[A-Za-z0-9_.-]+",
            _text(document["role"]),
        )
        is None
...
def _audit_config(value: object) -> dict[str, object]:
    ...
    for config in configs:
        ...
        if (
            item["logType"] not in {"ADMIN_READ", "DATA_READ", "DATA_WRITE"}
            or type(exempted) is not list
            or any(type(member) is not str for member in exempted)
        ):
            raise ValueError
        normalized.append(
            {"exemptedMembers": sorted(exempted), "logType": item["logType"]}
        )
    return {
        "auditLogConfigs": sorted(normalized, key=_canonical),
...
def _iam_policy(value: object) -> dict[str, object]:
    ...
    if (
        any("condition" in binding for binding in normalized_bindings)
        and document["version"] != 3
    ):
        raise ValueError
    return {
        "auditConfigs": sorted(
            (_audit_config(item) for item in audits), key=_canonical
        ),
...
def _membership(value: object) -> dict[str, str]:
    ...
    if (
        document["membership"] not in {"MEMBERSHIP_MATCHED", "MEMBERSHIP_NOT_MATCHED"}
...
def _allow_binding(value: object, binding: dict[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        or document["rolePermission"]
        not in {"ROLE_PERMISSION_INCLUDED", "ROLE_PERMISSION_NOT_INCLUDED"}
        ...
        raise ValueError
    memberships = {
        name: _membership(item) for name, item in document["memberships"].items()
    }
    combined = _membership(document["combinedMembership"])
    matched = any(
        item["membership"] == "MEMBERSHIP_MATCHED" for item in memberships.values()
    )
    if combined["membership"] != (
        "MEMBERSHIP_MATCHED" if matched else "MEMBERSHIP_NOT_MATCHED"
    ):
        raise ValueError
    ...
    if "condition" in binding:
        if (
            "condition" not in document
            or "conditionExplanation" not in document
            or _expr(document["condition"]) != binding["condition"]
        ):
            raise ValueError
        result["condition"] = binding["condition"]
        result["conditionExplanation"] = _condition_explanation(
            document["conditionExplanation"]
        )
        condition_passes = result["conditionExplanation"]["value"]
    elif "condition" in document or "conditionExplanation" in document:
        raise ValueError
    granted = (
        document["rolePermission"] == "ROLE_PERMISSION_INCLUDED"
        and matched
        and condition_passes
    )
    if document["allowAccessState"] != (
        "ALLOW_ACCESS_STATE_GRANTED" if granted else "ALLOW_ACCESS_STATE_NOT_GRANTED"
    ):
        raise ValueError
    return result
...
def _allow_policy(
    ...
    if (
        document["allowAccessState"]
        not in {"ALLOW_ACCESS_STATE_GRANTED", "ALLOW_ACCESS_STATE_NOT_GRANTED"}
        or document["relevance"] not in _RELEVANCE
        or not _text(document["fullResourceName"]).startswith("//")
    ):
        raise ValueError
    ...
    for binding, explanation in zip(policy["bindings"], explanations, strict=True):
        ...
        if role in by_role:
            ...
            counts[role] += 1
            if (
                not _expected_binding(binding, spec)
                or document["fullResourceName"]
                != "//cloudkms.googleapis.com/"
                + access.resource.rsplit("/cryptoKeyVersions/", 1)[0]
            ):
                raise ValueError
            expected_permission = access.permission in spec.permissions
            if (
                item["rolePermission"] == "ROLE_PERMISSION_INCLUDED"
            ) != expected_permission:
                raise ValueError
        ...
        if item["combinedMembership"]["membership"] == "MEMBERSHIP_MATCHED":
            if (
                role not in by_role
                or by_role[role].member != "serviceAccount:" + access.principal
            ):
                raise ValueError
        if (
            access.permission == "cloudkms.cryptoKeyVersions.useToSign"
            and item["rolePermission"] == "ROLE_PERMISSION_INCLUDED"
            and role != specs[0].role
        ):
            raise ValueError
    ...
    return (
        {
            "allowAccessState": document["allowAccessState"],
            "bindingExplanations": sorted(normalized, key=_canonical),
            "fullResourceName": document["fullResourceName"],
            "policy": policy,
...
def _allow(
    value: object, access: _AccessSpec, specs: tuple[_BindingSpec, ...]
) -> dict[str, object]:
    ...
    if any(count != 1 for count in counts.values()) or grants != (
        1 if access.required == "CAN_ACCESS" else 0
    ):
        raise ValueError
    expected_state = (
        "ALLOW_ACCESS_STATE_GRANTED" if grants else "ALLOW_ACCESS_STATE_NOT_GRANTED"
    )
    if document["allowAccessState"] != expected_state:
        raise ValueError
    if grants and not any(
        item["role"] == access.grant_role
        and item["allowAccessState"] == "ALLOW_ACCESS_STATE_GRANTED"
        for policy in normalized
        for item in policy["bindingExplanations"]
    ):
        raise ValueError
    return {
        "allowAccessState": expected_state,
        "explainedPolicies": sorted(normalized, key=_canonical),
        "relevance": document["relevance"],
    }
...
def _limited(
    value: object, maximum: int, pattern: str | None = None, minimum: int = 1
) -> str:
    _require(
        type(value) is str
        and minimum <= len(value) <= maximum
        and (pattern is None or fullmatch(pattern, value) is not None)
    )
    return value


def _deny_timestamp(value: object) -> tuple[datetime, int]:
    text = _limited(value, 30)
    _require(
        fullmatch(
            r"[0-9]{4}-(?:0[1-9]|1[0-2])-"
    ...
    return datetime.fromisoformat(text[:19] + "+00:00"), int(
        ((text[20:-1] if text[19:20] == "." else "") + "000000000")[:9]
    )


def _deny_values(
    value: object, pattern: str, maximum: int, required: bool
) -> list[str]:
    _require(
        type(value) is list and len(value) <= 500 and (not required or bool(value))
    )
    result = [_limited(item, maximum, pattern) for item in value]
    _require(len(result) == len(set(result)))
    return sorted(result)


def _deny_rule(value: object) -> dict[str, object]:
    document = _members(value, ("denyRule",), ("description",))
    rule = _members(
        document["denyRule"],
        ("deniedPermissions", "deniedPrincipals"),
        ("denialCondition", "exceptionPermissions", "exceptionPrincipals"),
    )
    result = {
        "deniedPermissions": _deny_values(
            rule["deniedPermissions"], _DENY_PERMISSION, 256, True
        ),
        "deniedPrincipals": _deny_values(
            rule["deniedPrincipals"], _DENY_PRINCIPAL, 2_048, True
        ),
        "exceptionPermissions": _deny_values(
            rule.get("exceptionPermissions", []), _DENY_PERMISSION, 256, False
        ),
        "exceptionPrincipals": _deny_values(
            rule.get("exceptionPrincipals", []), _DENY_PRINCIPAL, 2_048, False
        ),
    }
    _require("principalSet://goog/public:all" not in result["exceptionPrincipals"])
    if "denialCondition" in rule:
        result["denialCondition"] = _expr(rule["denialCondition"])
    return {
        "denyRule": result,
        **(
            {"description": _limited(document["description"], 256, minimum=0)}
...
def _deny_policy_text(value: object) -> dict[str, object]:
    document = _members(
        ...
        ("annotations", "deleteTime", "displayName"),
    )
    rules = document["rules"]
    annotations = document.get("annotations", {})
    _limited(document["name"], 256, _DENY_POLICY_NAME)
    _limited(document["uid"], 128)
    _require(
        document["kind"] == "DenyPolicy"
        and type(rules) is list
        and 1 <= len(rules) <= 500
        and "deleteTime" not in document
    )
    _b64_standard(document["etag"])
    _require(
        _deny_timestamp(document["updateTime"])
        >= _deny_timestamp(document["createTime"])
    )
    _require(
        type(annotations) is dict
        and len(annotations) <= 64
        and not any(
            type(key) is not str
            or not 1 <= len(key) <= 63
    ...
    normalized = {
        name: document[name]
        for name in ("createTime", "etag", "kind", "name", "uid", "updateTime")
    }
    normalized["rules"] = [_deny_rule(rule) for rule in rules]
    return {
        **normalized,
        **(
            {"displayName": _limited(document["displayName"], 63, minimum=0)}
...
def _deny_annotation(value: object, state: str) -> dict[str, object]:
    ...
    _require(
        document[state] in _JSON_ENUMS[state] and document["relevance"] in _RELEVANCE
    )
    return document
...
def _deny_map(value: object, state: str) -> dict[str, object]:
    _require(
        type(value) is dict
        and len(value) <= 500
        and not any(type(name) is not str or not name for name in value)
    )
...
def _deny_pair(
    ...
    combined = _deny_annotation(item[combined_name], state)
    members = _deny_map(item[map_name], state)
    matched_state = (
        "PERMISSION_PATTERN_MATCHED"
        if state == "permissionMatchingState"
        else "MEMBERSHIP_MATCHED"
    )
    matched = any(member[state] == matched_state for member in members.values())
    _require(
        set(members) == set(expected) and (combined[state] == matched_state) == matched
    )
    return combined, members, matched


def _deny_permission_matches(pattern: str, access: _AccessSpec) -> bool:
    service, member = pattern.split("/", 1)
    resource, action = member.split(".", 1)
    expected_resource, expected_action = access.permission.split(".", 1)[1].split(
        ".", 1
    )
    return (
        service == "cloudkms.googleapis.com"
        and resource in {"*", expected_resource}
        and action in {"*", expected_action}
    )
...
def _deny_principal_matches(pattern: str, access: _AccessSpec) -> bool | None:
    return (
        ...
        in {
            ...
            False
            if pattern.startswith(("principal://", "deleted:principal://"))
            else None
        )
    )
...
def _deny_policy(value: object, access: _AccessSpec) -> dict[str, object]:
    document = _members(
        value, ("denyAccessState", "policy", "relevance", "ruleExplanations")
    )
    policy = _deny_policy_text(document["policy"])
    explanations = document["ruleExplanations"]
    _require(
        document["denyAccessState"] in _JSON_ENUMS["denyAccessState"]
        and document["relevance"] in _RELEVANCE
        and type(explanations) is list
        and len(explanations) == len(policy["rules"])
    )
    normalized = []
    for policy_rule, explanation in zip(policy["rules"], explanations, strict=True):
        rule = policy_rule["denyRule"]
        item = _members(
            explanation, _DENY_RULE_MEMBERS, ("condition", "conditionExplanation")
        )
        _require(
            item["denyAccessState"] in _JSON_ENUMS["denyAccessState"]
            and item["relevance"] in _RELEVANCE
        )
        result = dict(item)
        denied_permission = _deny_pair(
            item,
            "combinedDeniedPermission",
            "deniedPermissions",
            "permissionMatchingState",
            rule["deniedPermissions"],
        )
        exception_permission = _deny_pair(
            item,
            "combinedExceptionPermission",
            "exceptionPermissions",
            "permissionMatchingState",
            rule["exceptionPermissions"],
        )
        denied_principal = _deny_pair(
            item,
            "combinedDeniedPrincipal",
            "deniedPrincipals",
            "membership",
            rule["deniedPrincipals"],
        )
        exception_principal = _deny_pair(
            item,
            "combinedExceptionPrincipal",
            "exceptionPrincipals",
            "membership",
            rule["exceptionPrincipals"],
        )
        for names, pair in zip(
            (
                ("combinedDeniedPermission", "deniedPermissions"),
                ("combinedExceptionPermission", "exceptionPermissions"),
                ("combinedDeniedPrincipal", "deniedPrincipals"),
                ("combinedExceptionPrincipal", "exceptionPrincipals"),
            ),
            (
                denied_permission,
            ...
            result[names[0]], result[names[1]] = pair[:2]
        _require(
            not any(
                (annotation["permissionMatchingState"] == "PERMISSION_PATTERN_MATCHED")
                != _deny_permission_matches(pattern, access)
                for pair in (denied_permission, exception_permission)
                for pattern, annotation in pair[1].items()
            )
        )
        _require(
            not any(
                (known := _deny_principal_matches(pattern, access)) is not None
                and (annotation["membership"] == "MEMBERSHIP_MATCHED") != known
                for pair in (denied_principal, exception_principal)
                for pattern, annotation in pair[1].items()
            )
        )
        condition = "denialCondition" in rule
        _require(
            ("condition" in item) == condition
            and ("conditionExplanation" in item) == condition
            and (not condition or _expr(item["condition"]) == rule["denialCondition"])
        )
        condition_explanation = (
            _condition_explanation(item["conditionExplanation"]) if condition else None
        )
        condition_matches = (
            condition_explanation["value"]
            if condition_explanation is not None
            else True
        )
        if condition:
            result["condition"] = rule["denialCondition"]
            result["conditionExplanation"] = condition_explanation
        denied = (
            denied_permission[2]
            and denied_principal[2]
            and not exception_permission[2]
            and not exception_principal[2]
            and condition_matches
        )
        _require(
            item["denyAccessState"]
            == (
                "DENY_ACCESS_STATE_DENIED" if denied else "DENY_ACCESS_STATE_NOT_DENIED"
            )
        )
        normalized.append(result)
    denied = any(
        rule["denyAccessState"] == "DENY_ACCESS_STATE_DENIED" for rule in normalized
    )
    _require(
        document["denyAccessState"]
        == ("DENY_ACCESS_STATE_DENIED" if denied else "DENY_ACCESS_STATE_NOT_DENIED")
    )
    return {
...
def _deny(value: object, access: _AccessSpec) -> dict[str, object]:
    ...
    if (
        document["denyAccessState"]
        not in {"DENY_ACCESS_STATE_DENIED", "DENY_ACCESS_STATE_NOT_DENIED"}
        or type(document["permissionDeniable"]) is not bool
        or document["relevance"] not in _RELEVANCE
    ...
    for resource in resources:
        ...
        if (
            item["denyAccessState"]
            not in {"DENY_ACCESS_STATE_DENIED", "DENY_ACCESS_STATE_NOT_DENIED"}
            or item["relevance"] not in _RELEVANCE
            or not _text(item["fullResourceName"]).startswith("//")
            or type(item["explainedPolicies"]) is not list
        ):
            raise ValueError
        policies = sorted(
            (_deny_policy(policy, access) for policy in item["explainedPolicies"]),
            key=_canonical,
        )
        policy_denied = any(
            policy.get("denyAccessState") == "DENY_ACCESS_STATE_DENIED"
            for policy in policies
            if type(policy) is dict
        )
        if item["denyAccessState"] != (
            "DENY_ACCESS_STATE_DENIED"
            if policy_denied
            else "DENY_ACCESS_STATE_NOT_DENIED"
        ):
            raise ValueError
        denied |= policy_denied
        normalized.append({**item, "explainedPolicies": policies})
    if document["denyAccessState"] != (
        "DENY_ACCESS_STATE_DENIED" if denied else "DENY_ACCESS_STATE_NOT_DENIED"
    ):
        raise ValueError
    return {
        "denyAccessState": document["denyAccessState"],
        "explainedResources": sorted(normalized, key=_canonical),
        "permissionDeniable": document["permissionDeniable"],
        "relevance": document["relevance"],
    }
...
def _effective_tags(value: object) -> list[dict[str, object]]:
    ...
    for tag in value:
        ...
        if (
            ...
            or fullmatch(r"tagKeys/[1-9][0-9]*", item["tagKey"]) is None
            or fullmatch(r"tagValues/[1-9][0-9]*", item["tagValue"]) is None
            or fullmatch(
                r"(?:organizations|projects)/[1-9][0-9]*", item["tagKeyParentName"]
            )
            is None
            or fullmatch(r"[A-Za-z0-9_-]+/[A-Za-z0-9._-]+", item["namespacedTagKey"])
            is None
            or fullmatch(
                r"[A-Za-z0-9_-]+/[A-Za-z0-9._-]+/[A-Za-z0-9._-]+",
                item["namespacedTagValue"],
            )
            is None
        ):
            raise ValueError
...
def _access_tuple(value: object, access: _AccessSpec) -> dict[str, object]:
    ...
    kind = (
        "CryptoKeyVersion" if "/cryptoKeyVersions/" in access.resource else "CryptoKey"
    )
    expected_resource = {
    ...
    if (
        document["principal"] != access.principal
        or document["fullResourceName"]
        != "//cloudkms.googleapis.com/" + access.resource
        or document["permission"] != access.permission
        or type(document["permissionFqdn"]) is not str
        or fullmatch(
            r"cloudkms\.googleapis\.com/[A-Za-z][A-Za-z0-9.]+",
            document["permissionFqdn"],
        )
        is None
        or _members(context["resource"], ("name", "service", "type"))
        != expected_resource
    ):
        raise ValueError
    return {
        **document,
        "conditionContext": {
            "effectiveTags": _effective_tags(context.get("effectiveTags", [])),
...
def _access(
    session: _EvidenceSession, access: _AccessSpec, specs: tuple[_BindingSpec, ...]
) -> dict[str, object]:
    kind = (
        "CryptoKeyVersion" if "/cryptoKeyVersions/" in access.resource else "CryptoKey"
    )
    ...
    body = _canonical(
        {
            "accessTuple": {
                "conditionContext": {"resource": resource},
                "fullResourceName": "//cloudkms.googleapis.com/" + access.resource,
    ...
    if (
        access.required == "CAN_ACCESS"
        and deny["denyAccessState"] != "DENY_ACCESS_STATE_NOT_DENIED"
    ):
        raise ValueError
    if (
        access.required == "CANNOT_ACCESS"
        and allow["allowAccessState"] != "ALLOW_ACCESS_STATE_NOT_GRANTED"
        and deny["denyAccessState"] != "DENY_ACCESS_STATE_DENIED"
    ):
        raise ValueError
    return {
        "accessTuple": _access_tuple(value["accessTuple"], access),
        "allowPolicyExplanation": allow,
        "denyPolicyExplanation": deny,
        "overallAccessState": access.required,
        "pabPolicyExplanation": _pab(value["pabPolicyExplanation"]),
    }


def _snapshot(
    observer: _KmsObserverClient,
    session: _EvidenceSession,
    manifest: _Manifest,
    bindings: tuple[_BindingSpec, ...],
    accesses: tuple[_AccessSpec, ...],
) -> dict[str, object]:
    ...
    roles = [
        _role(session, spec.role, spec.permissions, etag)
        for spec, etag in zip(bindings, manifest.etags, strict=True)
    ]
    evaluations = [_access(session, access, bindings) for access in accesses]
    return {
        "accessEvaluations": evaluations,
...
def _probe(signer: _KmsSignerClient, manifest: _Manifest) -> tuple[bytes, bytes]:
    request = kms_v1.AsymmetricSignRequest(
        name=manifest.resource, data=_PROBE, data_crc32c=_crc32c(_PROBE)
    )
    ...
    if (
        response.name != manifest.resource
    ...
    request_digest = sha256(
        _canonical(
            {
                "data": _b64url_text(_PROBE),
                "dataCrc32c": _crc32c(_PROBE),
                "name": manifest.resource,
            }
        )
    ).digest()
...
def _admit(
    ...
    if (
        finished < started
        or finished - started > _MAX_SPAN_US
        or finished > _MAX_UNIX_US - _LIFETIME_US
        or before != after
    ):
        raise ValueError
...
def admit_security_audit_observer_root(
    *,
    ...
    try:
        ...
        result = _admit(
            parsed, observer_client, signer_client, evidence_session, trusted_clock
        )
    except Exception:
        refused = True
    if refused:
