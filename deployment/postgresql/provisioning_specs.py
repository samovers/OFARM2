...
@dataclass(frozen=True, slots=True)
class TenantWriteLockSelectionOwnerAdmissionSealerSpec:
    """One-use V7 capsule for the tenant write-lock owner admission."""
    ...
    def source(self) -> str:
        return " ".join(
            (
                "BEGIN",
                "IF (SELECT pg_catalog.count(*) = 7 "
                "AND pg_catalog.max(version) = 7 "
                "AND pg_catalog.max(filename) FILTER (WHERE version = 7) = "
                "'0007_tenant_write_lock_selection_owner_admission.sql' "
                "AND pg_catalog.max(service_identity) "
                "FILTER (WHERE version = 7) = "
                "'ofarm.tenant-postgresql.v1' "
                f"FROM {self.ledger_schema_name}.{self.ledger_name}) "
                "IS DISTINCT FROM TRUE THEN",
                "RAISE EXCEPTION USING ERRCODE = '55000', "
                "MESSAGE = 'tenant write-lock selection-owner admission "
                "ordering marker differs';",
                "END IF;",
                "GRANT EXECUTE ON FUNCTION "
                f"{self.target_schema_name}.take_tenant_write_lock() "
                f"TO {self.owner_role};",
                f"GRANT CREATE ON SCHEMA {self.schema_name} "
                f"TO {self.execute_role};",
                f"ALTER FUNCTION {self.qualified_function}() SECURITY INVOKER;",
    ...
    def manifest(self) -> dict[str, object]:
        return {
            ...
            "grants": [
                {
                    "schema": self.target_schema_name,
                    "name": "take_tenant_write_lock",
                    "argumentTypes": [],
                    "grantee": self.owner_role,
                }
            ],
            "consumedAfterMigrationVersion": 7,
            "postConsumption": {
                "securityInvoker": True,
                "owner": self.execute_role,
                "droppedBeforeCommit": True,
...
class ProvisioningSpec:
    ...
    tenant_write_lock_selection_owner_admission_sealer: (
        TenantWriteLockSelectionOwnerAdmissionSealerSpec | None
    )
                ...
                **(
                    {
                        "tenantWriteLockSelectionOwnerAdmissionSealer": (
                            self.tenant_write_lock_selection_owner_admission_sealer.manifest()
                        )
                    }
                    if self.tenant_write_lock_selection_owner_admission_sealer
                    is not None
                    else {}
                ),
                **(
...
_TENANT_WRITE_LOCK_SELECTION_OWNER_ADMISSION_SEALER = (
    TenantWriteLockSelectionOwnerAdmissionSealerSpec(
        schema_name="ofarm_infrastructure",
        function_name="seal_tenant_write_lock_selection_owner_admission",
        execute_role="ofarm_migrator",
        ledger_schema_name="ofarm",
        ledger_name="schema_migration",
        target_schema_name="ofarm",
...
SECURITY_AUDIT_PROVISIONING_SPEC = ProvisioningSpec(
    ...
    tenant_write_lock_selection_owner_admission_sealer=None,
...
def _validate_spec(spec: ProvisioningSpec) -> None:
    ...
    write_lock_owner_admission_sealer = (
    ...
    if spec == TENANT_PROVISIONING_SPEC:
        ...
        if (
            ...
            raise ProvisioningSpecError(
                "tenant write-lock selection-owner admission sealer is not exact"
            )
        ...
        or write_lock_owner_admission_sealer is not None
    ...
    if write_lock_owner_admission_sealer is not None:
        for value, label in (
            (write_lock_owner_admission_sealer.schema_name, "write-lock sealer schema"),
            (
                write_lock_owner_admission_sealer.function_name,
                "write-lock sealer function",
            ),
            (write_lock_owner_admission_sealer.execute_role, "write-lock sealer caller"),
            (
                write_lock_owner_admission_sealer.ledger_schema_name,
                "write-lock sealer ledger schema",
            ),
            (
                write_lock_owner_admission_sealer.ledger_name,
                "write-lock sealer ledger",
            ),
            (
                write_lock_owner_admission_sealer.target_schema_name,
                "write-lock sealer target schema",
            ),
            (write_lock_owner_admission_sealer.owner_role, "write-lock sealer grantee"),
        ):
            _validate_identifier(value, label)
        if (
            write_lock_owner_admission_sealer.schema_name,
            write_lock_owner_admission_sealer.function_name,
            write_lock_owner_admission_sealer.execute_role,
            write_lock_owner_admission_sealer.ledger_schema_name,
            write_lock_owner_admission_sealer.ledger_name,
            write_lock_owner_admission_sealer.target_schema_name,
            write_lock_owner_admission_sealer.owner_role,
        ) != (
            lock.schema_name,
            ...
            raise ProvisioningSpecError(
                "tenant write-lock selection-owner admission sealer identity differs"
            )
